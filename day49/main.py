"""
HACK: GoogleのOAuthでログインする。
    １．そして、OAuthの認証をブラウザ操作で行うのが難しい。（Googleの認証ボタンの取得がムズイ(一回、一回要素が変化する or クラス名が変わる（ex)buttonタグ、iframeタグ..etc）)。）
    なんでどうするか考える
    ２．Edgeを使っているからなのか？使用しているブラウザが安全でないというエラーがでる。
